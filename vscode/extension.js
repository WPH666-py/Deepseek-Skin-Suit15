// DeepSkin Suit15 — DeepSeek 蓝色大肥鱼皮肤套件15 (VSCode / Trae / CodeX)
...
const REPO_URL = 'https://github.com/WPH666-py/Deepseek-Skin-Suit15.git';
...
function detectPython() {
  const cfg = vscode.workspace.getConfiguration('deepskin15').get('pythonPath', '');
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('deepskin15').get('repoPath', ''));
  const cands = [
    cfg,
    path.join(os.homedir(), '.deepskin-suit15'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'DeepSkin-Suit15') : null,
    path.join(os.homedir(), 'DeepSkin-Suit15')
  ].filter(Boolean);
...
function envError(msg) {
  vscode.window.showWarningMessage(
    ...
    path.join(process.env.USERPROFILE || os.homedir(), 'DeepSkin-Suit15') + ' 并安装 Python3 + Pillow。'
  );
}
...
function runWallpaper(args) {
  ...
  const repo = findRepo();
  if (!repo) return envError('未找到皮肤套件15仓库。');
  ...
  child.on('close', (code) => {
    if (code === 0) {
      vscode.window.setStatusBarMessage('🐳 大肥鱼15壁纸已应用!', 5000);
    } else {
...
class GalleryProvider {
  ...
  resolveWebviewView(view) {
    this._view = view;
    const ext = vscode.extensions.getExtension('wp666.deepskin-suit15');
...
function html(thumb, repo) {
  ...
  const ready = repo ? '' : '未检测到皮肤套件15仓库, 请先克隆 ' + REPO_URL;
  return (
...
function activate(ctx) {
  ...
  register('deepskin15.setGrid', () => runWallpaper(['grid']));
  register('deepskin15.setSingle1', () => runWallpaper(['single1']));
  register('deepskin15.setSingle2', () => runWallpaper(['single2']));
  register('deepskin15.setSingle3', () => runWallpaper(['single3']));
  register('deepskin15.setSingle4', () => runWallpaper(['single4']));
  register('deepskin15.setRandom', () => runWallpaper(['random']));
  register('deepskin15.openSwitcher', () => spawnGui('switcher.py'));
  register('deepskin15.startPet', () => spawnGui('pet.py'));
  register('deepskin15.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.deepskin15')
  );
  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('deepskin15.gallery', new GalleryProvider(), {
